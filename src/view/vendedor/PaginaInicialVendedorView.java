...
import controller.vendedor.PaginaInicialVendedorController;
import model.Livro;
...
public class PaginaInicialVendedorView extends JFrame {
   
    private Point pontoInicial;
    private JPanel painelDeConteudoPrincipal;
    private PaginaInicialVendedorController controller;
    private Cores cores;
    ...
    public PaginaInicialVendedorView() {
    	
    }
    public PaginaInicialVendedorView(String nomeUsuario) {
        controller = new PaginaInicialVendedorController(this);
    ...
    private JPanel criarSidebar() {
        ...
        String[] botoes = {"Home", "Livros","Vendas", "Lista Vendas","Meus Dados","Sair"};
    ...
    public JPanel criarPaginaInicial() {
        ...
        String totalFuncionarios = String.valueOf(usuarioService.totalFuncionarios());
        
        
        
        Usuario usuario = usuarioService.buscarPorUsuario(nomeUsuario);
        
        
        
        double total = 0;
        for(int i = 0; i < usuario.getVendasPorFuncionario().size(); i++) {
        	total += usuario.getVendasPorFuncionario().get(i).getPagamento().getValorTotalDaVenda();
        }
        
        int somaLivrosV = 0;
        for(int i = 0; i < usuario.getVendasPorFuncionario().size(); i++) {
        	somaLivrosV += usuario.getVendasPorFuncionario().get(i).getItens().size();
        }
        ...
        cards.add(createCard("Vendas do Mes", String.format("%.2f", total)));
    ...
    private JPanel criarTabelaUltimasVendas(List<Venda> vendas) {
        String[] colunas = {"Usuario", "Total Livros", "Valor", "Data"};
        ...
        if (vendas != null && !vendas.isEmpty()) {
            for (Venda venda : vendas) {
               ...
               if(venda.getFuncionario().getUsuario().equalsIgnoreCase(nomeUsuario)) {
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> new PaginaInicialVendedorView("Mr. Robot"));
    }
}